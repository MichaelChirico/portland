...
l1=0;l2=1e-5;lambda=.5;delta=1;T0=0;pp=.5
...
grid.translations = CJ(rho_x = seq(0, 1, length.out = 5),
                       rho_y = seq(0, 1, length.out = 5))

scores = copy(grid.translations)[ , c('pei', 'pai') := NA_real_]

for (ii in seq_len(nrow(grid.translations))) {
  rho_x = grid.translations[ii, rho_x]
  rho_y = grid.translations[ii, rho_y]
  xrng[1L] = xrng[1L] - rho_x*delx
  yrng[1L] = yrng[1L] - rho_y*dely
