...
ff = list.files("scores", full.names = TRUE, pattern = "csv")
    ...
    .SD[dimcombs[ii], on = c("delx", "dely"),
        {
          ...
          idx = which(pai >= .9*max(pai))
          text(lt[idx], eta[idx], round(pai[idx]), cex = .8)
          if (any(idx <- pai >= .9*MAX))
            text(lt[idx], eta[idx], round(pai[idx]), cex = .8, col = "red")
        }]
  }
...
#PEI
scores[ , {
  pdf(paste0("scores/pei_", .BY[[1L]], .BY[[2L]], ".pdf"))
  par(mfrow = c(uniqueN(delx), uniqueN(dely)),
      mar = c(0, 0, 0, 0),
      oma = c(5.1, 4.1, 3.1, 2.1))
  MAX = max(pei)
    ...
    .SD[dimcombs[ii], on = c("delx", "dely"),
        {
          ...
          if (ii %% 3L == 0L) mtext(paste0("N-S Cell Size: ", dely[1L]),
                                    side = 4L, cex = .5)
          tile.axes(ii, 3L, 3L, params = axparams)
          idx = which(max(pei) - pei <= .01)
          text(lt[idx], eta[idx], round(pei[idx], 2), cex = .8)
          if (any(idx <- MAX - pei <= .01))
            text(lt[idx], eta[idx], round(pei[idx], 2), cex = .8, col = "red")
        }]
  }
  mtext(side = 1L, "Time Lengthscale (Weeks)",
        outer = TRUE, line = 2.5)
  mtext(side = 2L, "Space Lengthscale (Factor of Dimension)",
        outer = TRUE, line = 2.5)
  title(paste0("PEI Sensitivity: ", .BY[[1L]], "/", .BY[[2L]]),
        outer = TRUE)
  dev.off()
}, by = .(crime, horizon)]
